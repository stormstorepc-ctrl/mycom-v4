    ...
    try {
        ...
        const existingRoom = await db.query(
            `SELECT * FROM chat_rooms 
             WHERE user_id = $1 AND shop_id = $2 AND (quote_id = $3 OR quote_id IS NULL)
             LIMIT 1`,
            [req.user.id, shop_id, quote_id]
        );
        ...
        const result = await db.query(
            `INSERT INTO chat_rooms (user_id, shop_id, quote_id) 
             VALUES ($1, $2, $3) 
             RETURNING *`,
            [req.user.id, shop_id, quote_id]
        );
        ...
        if (req.user.role === 'shop') {
            ...
            result = await db.query(
                `SELECT cr.*, 
                        ...
                        s.shop_name,
                        (SELECT message FROM messages m WHERE m.room_id = cr.id ORDER BY m.created_at DESC LIMIT 1) as last_message,
                        (SELECT created_at FROM messages m WHERE m.room_id = cr.id ORDER BY m.created_at DESC LIMIT 1) as last_message_time,
                        (SELECT COUNT(*) FROM messages m WHERE m.room_id = cr.id AND m.is_read = FALSE AND m.sender_type != 'shop') as unread_count
                 FROM chat_rooms cr
            ...
            result = await db.query(
                `SELECT cr.*, 
                        s.shop_name as other_party_name,
                        (SELECT message FROM messages m WHERE m.room_id = cr.id ORDER BY m.created_at DESC LIMIT 1) as last_message,
                        (SELECT created_at FROM messages m WHERE m.room_id = cr.id ORDER BY m.created_at DESC LIMIT 1) as last_message_time,
                        (SELECT COUNT(*) FROM messages m WHERE m.room_id = cr.id AND m.is_read = FALSE AND m.sender_type != 'user') as unread_count
                 FROM chat_rooms cr
                 JOIN shops s ON s.id = cr.shop_id
                 WHERE cr.user_id = $1
                 ORDER BY last_message_time DESC NULLS LAST`,
...
router.get('/rooms/:roomId/messages', authenticate, async (req, res) => {
    try {
        ...
        if (req.user.role === 'shop') {
            ...
            if (shopResult.rows[0]?.id !== room.shop_id) {
    ...
    try {
        ...
        const result = await db.query(
            `INSERT INTO messages (room_id, sender_id, sender_type, message) 
             VALUES ($1, $2, $3, $4) 
             RETURNING *`,
            [roomId, req.user.id, req.user.role === 'shop' ? 'shop' : 'user', message]
        );
