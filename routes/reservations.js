...
router.put('/:id/status',authenticate,async(req,res)=>{try{const{id}=req.params,{status}=req.body;if(!['pending','confirmed','completed','cancelled'].includes(status))return res.status(400).json({error:'올바르지 않은 상태입니다.'});const rr=await db.query('SELECT * FROM reservations WHERE id=$1',[id]);if(!rr.rows.length)return res.status(404).json({error:'예약을 찾을 수 없습니다.'});const reservation=rr.rows[0];if(req.user.role==='user'&&reservation.user_id!==req.user.id)return res.status(403).json({error:'예약 수정 권한이 없습니다.'});if(req.user.role==='shop'){const s=await db.query('SELECT user_id FROM shops WHERE id=$1',[reservation.shop_id]);if(s.rows[0]?.user_id!==req.user.id)return res.status(403).json({error:'예약 수정 권한이 없습니다.'})}const r=await db.query(`UPDATE reservations SET status=$1,updated_at=NOW() WHERE id=$2 RETURNING *`,[status,id]);if(status==='confirmed'||status==='completed')await db.query(`INSERT INTO notifications(user_id,type,title,message,link) VALUES($1,$2,$3,$4,$5)`,[reservation.user_id,'reservation_status','예약 상태 변경',`예약 상태가 '${status}'로 변경되었습니다.`,`/my-activity.html?tab=reservations`]);res.json({reservation:r.rows[0]})}catch(e){res.status(500).json({error:'예약 상태 업데이트 오류'})}});
