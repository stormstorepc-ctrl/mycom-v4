...
router.post('/register', async (req, res) => {
    try {
        ...
        const existingUser = await db.query('SELECT id FROM users WHERE LOWER(email) = $1', [email]);
...
router.post('/register-shop', async (req, res) => {
    try {
        ...
        const email = String(req.body.email || '').trim().toLowerCase();
        const { password, name, phone, shop_name, business_number, address, detail_address, latitude, longitude, shop_phone, open_kakao_url, description } = req.body;
