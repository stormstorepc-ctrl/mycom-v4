...
router.post('/register-shop', async (req, res) => {
    try {
        ...
        const configuredCode = String(process.env.PARTNER_INVITE_CODE || '').trim();
        if (!configuredCode || !inviteCode || inviteCode !== configuredCode) return res.status(403).json({ error: '유효한 MYCOM 업체 초대 링크로만 가입할 수 있습니다.' });
